...
static int dmatest_slave_func(void *data)
{
	...
	for (i = 0; i < src_cnt; i++) {
		...
		memset(thread->srcs[i], i+5, test_buf_size);
		if (!thread->srcs[i])
			goto err_srcbuf;
	}
	...
	while (!kthread_should_stop()
		...
		thread->rx_vir = dma_alloc_coherent(rx_dev->dev, len*bd_cnt,
						 &thread->rx_phy, GFP_ATOMIC);
   		if (thread->rx_vir == NULL) {
		      printk("axi: DMA tx alloc error\n");
		      return -1;
   		}
		memset(thread->rx_vir, 0, len*bd_cnt);
		
		rxd = rx_dev->device_prep_dma_cyclic(rx_chan, thread->rx_phy,
						len*bd_cnt, len, DMA_DEV_TO_MEM, flags);
		...
		printk("Verifying the data\n\r");
		for (i = 0;i < bd_cnt; i++) {
			for (j=0; j < test_buf_size; j++) {
				if (thread->rx_vir[j+rx_data] != i+5)
					printk("Data mismatch %x\n\r and i is %x\n\r",
						thread->rx_vir[j+rx_data], i);
			}
			rx_data += test_buf_size;
		}
		printk("Cyclic DMA test case passed\n\r");
	}

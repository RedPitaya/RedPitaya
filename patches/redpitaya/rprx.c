...
static long rprx_ioctl(struct file *file, unsigned int cmd , unsigned long arg) {
  ...
  //  rx->tmo = msecs_to_jiffies(3000000);
    smp_rmb();
    printk("rprx: ioctl cmd:%d arg%d\n",cmd,(int)arg);
    switch(cmd){
    case STOP_RX:{
        printk("rprx: ioctl terminate all rx\n");
    ...
    {   
        printk("rprx: ioctl cyclic rx s:0x%lx c:0x%x\n",rx->segment_size,rx->segment_cnt);
        smp_rmb();
        ...
        if(!rx->d){printk("rprx: rxd not set properly\n");}
        else{
        init_completion(&rx->cmp);
        rx->d->callback = rprx_slave_callback; //set completion callback
        rx->d->callback_param = rx;
        rx->cookie = rx->d->tx_submit(rx->d);    
        printk("rprx: submit\n");
            
        if(dma_submit_error(rx->cookie)){
            printk("rprx submit error %d \n", rx->cookie);
        }
        rx->flag=0;
    ...
    case SINGLE_RX:{
        printk("rprx: rx single dma s:0x%lx c:0x%x\n",rx->segment_size,rx->segment_cnt);smp_rmb();
        ...
        if(!rx->d){printk("rprx: rxd not set properly\n");}
            ...
            rx->d->callback_param = &rx;
            rx->cookie = rx->d->tx_submit(rx->d);
            printk("rprx: rx submit \n");
        }
        printk("rprx:rx dma_async_issue_pending \n");
        dma_async_issue_pending(rx->chan);    
    break;
    }
    case SET_RX_SGMNT_CNT:{
        ...
        printk("rprx: ioctl rx segment cnt set to 0x%x \n",rx->segment_cnt);
   }break;
    case SET_RX_SGMNT_SIZE:{
        ...
        printk("rprx: ioctl rx segment size set to 0x%lx \n",rx->segment_size);
        
    }break;  
    default:
         printk("rprx: ioctl %d-%lx not inmplemented \n",cmd,arg);
...
static struct file_operations fops = {
    .owner    = THIS_MODULE,
    .read = rprx_read, //it shall inform user of data availibility using blocking read
    .open     = rprx_open, 
    .release  = rprx_release, //dealocation of dma buffers
    .unlocked_ioctl = rprx_ioctl, //start stop functionality per channel
    .mmap    = rprx_mmap, //enables zerro copy from device to user
    .write = rprx_write,
};
...
static int rprx_probe(struct platform_device *pd)
{
    ...
    if(!rx){
        printk("rprx: no memory for device structrure\n");
        return -ENOMEM;
        }
    rx->rpdev=pd;
    ...
    if(rx->addrv==NULL){
    printk("rprx: ERROR DMA reserved memory not allocated\n");
    goto rmdev;
    }else {
		printk("rprx: reserved dma: %p and 0x%x KiB @0x%x\n",(void*)rx->chan,(RX_SGMNT_CNT*RX_SGMNT_SIZE)/1024,rx->addrp);
	}

...
static long rpdma_ioctl(struct file *file, unsigned int cmd , unsigned long arg) {
    ...
    case SET_TX:{
        ...
        }else if(arg==SINGLE){//single
            printk("rpdma:tx single shot dma \n");
    ...
    case SET_RX://rx prepair
    {
        if(arg==CYCLIC){    
            printk("rpdma:ioctl cyclic rx\n");

            rxd = rx_dev->device_prep_dma_cyclic(rx_chan,rpdma_tx_addrp, rx_segment_size*rx_segment_cnt, rx_segment_size,DMA_DEV_TO_MEM, DMA_CTRL_ACK | DMA_PREP_INTERRUPT);
            ...
            if(dma_rx_segment){
                printk("rpdma:dma_rx_segment not set properly\n");
            }
            else{
            rxd = dmaengine_prep_slave_single(rx_chan,dma_rx_segment, rx_segment_size*rx_segment_cnt, DMA_DEV_TO_MEM, DMA_CTRL_ACK | DMA_PREP_INTERRUPT);
			if(!rxd){
				printk("rpdma:rxd not set properly\n");
            }
            else{
				init_completion(&rx_cmp);
    ...
    case START_TX:{//start tx
            dma_async_issue_pending(tx_chan);
    } break;
       
    case START_RX:{//start rx
   ...
   case SIMPLE_RX:{
	...
	//	} 
	//	else  {
	//		printk("tx got completion callback. Status is \'%s\'\n",status == DMA_ERROR ? "error" : "in progress");
	//	}
	//	if(rpdma_tx_addrp)	
	//	dma_unmap_single(tx_dev->dev, rpdma_tx_addrp, tx_segment_size*tx_segment_cnt, DMA_MEM_TO_DEV);
...
static int rpdma_probe(struct platform_device *pdev)
{
    ...
    rpdma_tx_addrv =  dma_alloc_coherent(tx_dev->dev, tx_segment_size*tx_segment_cnt, &rpdma_tx_addrp, GFP_ATOMIC);
    if(rpdma_tx_addrv==NULL){
        printk("rpdma: ERROR tx not allocated\n");
    }else printk("rpdma: reserved %ld for tx \n",tx_segment_size*tx_segment_cnt);
    rpdma_rx_addrv=  dma_alloc_coherent(rx_dev->dev, rx_segment_size*rx_segment_cnt, &rpdma_rx_addrp, GFP_ATOMIC);
    if(rpdma_tx_addrv==NULL){
    printk("rpdma: ERROR rx not allocated\n");
    }else printk("rpdma: reserved %ld for rx \n",rx_segment_size*rx_segment_cnt);
    return 0;
...
static int rpdma_remove(struct platform_device *pdev)
{
    ...
    if(rpdma_rx_addrv){
       dma_free_coherent(NULL, rx_segment_size, rpdma_rx_addrv, rpdma_handle);
    }
    
    if(rpdma_tx_addrv){
       dma_free_coherent(NULL,tx_segment_size, rpdma_tx_addrv, rpdma_handle);
    }
    return 0;
}

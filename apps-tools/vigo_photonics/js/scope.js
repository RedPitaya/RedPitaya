...
(function(OSC, $, undefined) {
    ...
    OSC.param_callbacks = {};
    OSC.config = {};
    OSC.config.app_id = 'vigo_photonics';
    ...
    /** Paints a reading by how far it has gone: plain, then a warning, then an
     *  alarm. Used for the system block, where the numbers only matter when
     *  they get large. */
    OSC.setLevelColor = function(selector, value, warn, alarm) {
        let element = $(selector);
        element.toggleClass('level-warn', value >= warn && value < alarm);
        element.toggleClass('level-alarm', value >= alarm);
    };
    ...
    var performanceHandler = function() {
        ...
        // A value that is getting into trouble is worth seeing at a glance.
        let free_ratio = OSC.g_TotalMemory > 0 ? OSC.g_FreeMemory / OSC.g_TotalMemory : 1;
        OSC.setLevelColor('#cpu_load', OSC.g_CpuLoad, 70, 90);
        OSC.setLevelColor('#cpu_temp', OSC.g_Temperature, 70, 80);
        OSC.setLevelColor('#usagemem_view', 1 - free_ratio, 0.8, 0.9);
    ...
    OSC.updateJoystickPosition = function(){
        // The joystick and the buffer buttons sit at the top of the menu
    ...
    OSC.downloadFile = function(new_params){
        ...
        if (!filename.includes("error")) {
            if (filename !== ""){
                ...
                download("/vigo_photonics/files/"+filename,filename);
            }
        }
        else{
    ...
    OSC.exitEditing = function(noclose,event) {
        ...
        // The controller panels use radio groups whose buttons carry the
        // parameter name with the index of the button appended.
        if (id.startsWith("PTCC_") && !(id in OSC.params.orig)) {
            id = id.replace(/[0-9]+$/, "")
        }
    ...
    OSC.drawGraphGrid = function() {
        ...
        var canvas_height = OSC.ptccCanvasHeight();

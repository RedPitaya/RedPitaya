(function(OSC, $, undefined) {
    ...
    OSC.setScale = function(ch,new_params) {
        ...
        // The accumulated traces are drawn against this scale, so they only
        // stop meaning anything when it actually changes. The value is also
        // sent again on occasions that leave it alone, and throwing the traces
        // away then would lose a measurement for nothing.
        var previous = OSC.params.old[param_name] === undefined ? undefined
                                                                : OSC.params.old[param_name].value
        if (previous !== new_params[param_name].value)
    ...
    OSC.setTimeScale = function(new_params){
        ...
        // Sent again whenever the time cursors have to be recomputed, which
        // happens on a time offset change as well; the traces survive that.
        var previous = OSC.params.old['OSC_TIME_SCALE'] === undefined ? undefined
                                                                      : OSC.params.old['OSC_TIME_SCALE'].value
        if (previous !== new_params['OSC_TIME_SCALE'].value)
    ...
    OSC.setCurrentBuffer = function(new_params){
        if (OSC.params.orig['OSC_BUFFER_CURRENT']){
            ...
            // Zero is the newest capture, the rest count backwards from it.
            // A running oscilloscope always shows the newest one.
            var running = OSC.params.orig['OSC_RUN'] ? OSC.params.orig['OSC_RUN'].value : true
            $('#OSC_BUFFER_CURRENT_INFO').text(running || cur === 0 ? 'latest' : cur)

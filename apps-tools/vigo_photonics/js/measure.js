    ...
    OSC.handleMeasureList = function() {
        ...
        $('#meas_list .meas-item').each(function(index, elem) {
            ...
            if (item_val !== null) {
                ...
                // Same shape as the other blocks of the information row: a
                // label on the left, the value against the right edge.
                $('#info-meas').append(
                    '<div><span class="k"><span style="color:' + OSC.config.graph_colors[$elem.data('signal').toLowerCase()] + '">' + sig_name + '</span>/' + $elem.data('operator') + '</span><span class="v"><span id="OSC_MEAS_VAL' + mi_count + '">-</span>&nbsp;<span id="OSC_MEAS_UNITS">' + u + '</span><span id="OSC_MEAS_ORIG_UNITS" style="display:none;">' + u + '</span><span id="OSC_MEAS_ORIG_FOO" style="display:none;">' + $elem.data('operator') + '</span><span id="OSC_MEAS_ORIG_SIGNAME" style="display:none;">' + sig_name + '</span></span></div>'
                );
        ...
        // An empty block would be a frame with a title and nothing in it.
        $('#meas_group').prop('hidden', $('#info-meas').children().length === 0);

...
void CLACallbackHandler::captureStatus(rp_la::CLAController* controller, bool isTimeout, uint32_t numBytes, uint64_t numSamples, uint64_t preSamples, uint64_t postSamples) {
    if (isTimeout == false) {
        if (numSamples) {
...
void CLACallbackHandler::decodeStatus(rp_la::CLAController* controller, uint32_t numBytes, uint64_t numSamples, uint64_t preSamples, uint64_t postSamples) {
    if (numSamples) {
        controller->saveCaptureDataToFile(FILE_NAME);
        FILE* f = fopen(FILE_NAME_TRIGGER, "wb");
        if (!f) {
...
int rp_app_init(void) {
    setHomeSettingsPath("/.config/redpitaya/apps/la_pro_" + std::to_string((int)getModel()) + "/");
...
void updateFromFront(bool force) {
    ...
    auto getType = [](std::string name) {
        if (name == "CAN")
            return LA_DECODER_CAN;
        if (name == "I2C")
            return LA_DECODER_I2C;
        if (name == "SPI")
            return LA_DECODER_SPI;
        if (name == "UART")
            return LA_DECODER_UART;

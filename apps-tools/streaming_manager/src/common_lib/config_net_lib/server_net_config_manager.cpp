...
ServerNetConfigManager::ServerNetConfigManager(std::string defualt_file_settings_path,
...
auto ServerNetConfigManager::receiveCommand(uint32_t command, std::string tag) -> void
{
	...
	if (c == CNetConfigManager::ECommands::CS_REQUEST_SERVER_MEM_BLOCK_SIZE) {
		m_pNetConfManager->sendCommand(CNetConfigManager::ECommands::CS_RESPONSE_MEM_BLOCK_SIZE,
									   std::to_string(m_settings.getMemoryBlockSize()));
	}

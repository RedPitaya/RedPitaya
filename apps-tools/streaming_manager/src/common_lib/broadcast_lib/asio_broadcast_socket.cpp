...
auto CAsioBroadcastSocket::create(EModel model, std::string host,  std::string port) -> CAsioBroadcastSocket::Ptr {
    return std::make_shared<CAsioBroadcastSocket>(model,host, port);
}

CAsioBroadcastSocket::CAsioBroadcastSocket(EModel model,std::string host, std::string port) :
...
auto CAsioBroadcastSocket::initServer(EMode mode,int sleep_time_ms) -> void {
    ...
    if (!error){
        ...
        m_pimpl->m_socket->async_send_to(asio::buffer(buf.c_str(),buf.size()),senderEndpoint,std::bind(&CAsioBroadcastSocket::handlerSend, this, std::placeholders::_1 ,std::placeholders::_2 ));
    }else{
        errorNotify(error);
    }
}
...
auto CAsioBroadcastSocket::initClient() -> void {
    ...
    m_pimpl->m_socket->set_option(asio::ip::udp::socket::reuse_address(true));
    m_pimpl->m_socket->set_option(asio::socket_base::broadcast(true));
    m_pimpl->m_socket->async_receive(asio::buffer(m_SocketReadBuffer, SOCKET_BUFFER_SIZE),std::bind(&CAsioBroadcastSocket::handlerReceive, this, std::placeholders::_1 ,std::placeholders::_2 ));
}
...
auto CAsioBroadcastSocket::handlerReceive(const asio::error_code &error,size_t bytes_transferred) -> void {
    if (!error){
        ...
        asio::ip::udp::endpoint senderEndpoint(asio::ip::address_v4::any(), std::stoi(m_port));
        m_pimpl->m_socket->async_receive(

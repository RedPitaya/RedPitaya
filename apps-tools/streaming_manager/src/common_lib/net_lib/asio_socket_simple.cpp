...
auto CAsioSocketSimple::initServer() -> void {
    ...
    auto tcp_socket = std::make_shared<asio::ip::tcp::socket>(m_asio->getIO());
    ...
    m_tcp_acceptor->async_accept(*tcp_socket, m_tcp_endpoint, std::bind(&CAsioSocketSimple::handlerAccept, this, std::placeholders::_1, tcp_socket));
    m_mode = EMode::M_SERVER;
}
...
auto CAsioSocketSimple::initClient() -> void {
    ...
    m_tcp_socket->async_connect(m_tcp_endpoint, std::bind(&CAsioSocketSimple::handlerConnect, this, std::placeholders::_1));
...
auto CAsioSocketSimple::handlerAccept(const asio::error_code& _error, socket_ptr socket) -> void {
    if (!_error) {
        if (m_tcp_socket) {
            m_tcp_socket->cancel();
            m_tcp_socket->close();
...
auto CAsioSocketSimple::handlerSend(const asio::error_code& _error, size_t _bytesTransferred) -> void {
    ...
    if (!_error) {
        // MODIFY LATER
    } else if ((_error == asio::error::eof) || (_error == asio::error::connection_reset) || _error == asio::error::broken_pipe) {
        errorNotify(_error);
        // if (m_mode == EMode::M_SERVER && !m_disableRestartServer) {
        //     initServer();
        // }

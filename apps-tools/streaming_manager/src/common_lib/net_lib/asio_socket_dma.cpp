...
void CAsioSocketDMA::initServer() {
    ...
    m_tcp_acceptor->async_accept(*m_tcp_socket, m_tcp_endpoint, std::bind(&CAsioSocketDMA::handlerAcceptFromClient, this, std::placeholders::_1));
...
auto CAsioSocketDMA::initClient() -> void {
    ...
    m_tcp_socket->async_connect(m_tcp_endpoint, std::bind(&CAsioSocketDMA::handlerConnectToServer, this, std::placeholders::_1));

    m_mode = net_lib::EMode::M_CLIENT;
}

void CAsioSocketDMA::handlerConnectToServer(const asio::error_code& _error) {
    try {
        if (!_error) {
    ...
    if (m_tcp_socket) {
        for (auto i = (int)DataLib::EDataBuffersPackChannel::CH1; i <= (int)DataLib::EDataBuffersPackChannel::CH4; i++) {
            ...
            if (buff != NULL) {
                ...
                uint32_t offset = 0;
                while (needSendSize) {
                    uint32_t sendedSize = (uint32_t)m_tcp_socket->send(asio::buffer((char*)buff->getMappedMemory() + offset, needSendSize), 0, _error);
                    offset += sendedSize;

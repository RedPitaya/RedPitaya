...
auto CAsioNetSimple::create(net_lib::EMode _mode, std::string _host, uint16_t _port) -> CAsioNetSimple::Ptr {
    return std::make_shared<CAsioNetSimple>(_mode, _host, _port);
}

CAsioNetSimple::CAsioNetSimple(net_lib::EMode _mode, std::string _host, uint16_t _port) : m_mode(_mode), m_host(_host), m_port(_port), m_IsRun(false) {
    m_server = new CAsioSocketSimple(m_host, m_port);

    m_server->connectNotify.connect([&](auto& host) { this->connectNotify(host); });

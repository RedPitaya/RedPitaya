...
public:
	...
	std::enable_if_t<trait::is_callable_v<arg_list, Pmf, Ptr> && trait::is_observer_v<Ptr>, connection> connect(Pmf &&pmf,
																												Ptr &&ptr,
																												group_id gid = 0)
	{
	...
	/**
			 ...
			 */
	template<typename Pmf, typename Ptr>
	std::enable_if_t<trait::is_callable_v<arg_list, Pmf, Ptr> && !trait::is_observer_v<Ptr> && !trait::is_weak_ptr_compatible_v<Ptr>,
					 connection>
	...
	/**
			 ...
			 */
	template<typename Pmf, typename Ptr>
	std::enable_if_t<!trait::is_callable_v<arg_list, Pmf> && trait::is_weak_ptr_compatible_v<Ptr>, connection> connect(Pmf &&pmf,
																													   Ptr &&ptr,
																													   group_id gid = 0)
	{
		using trait::to_weak;
	...
	std::enable_if_t<trait::is_callable_v<arg_list, Callable> && trait::is_weak_ptr_compatible_v<Trackable>, connection> connect(
		Callable &&c, Trackable &&ptr, group_id gid = 0)
	{
		using trait::to_weak;

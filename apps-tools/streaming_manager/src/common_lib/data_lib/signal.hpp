...
class slot_pmf final : public slot_base<Args...> {
   public:
    ...
    constexpr slot_pmf(cleanable& c, F&& f, P&& p, group_id gid) : slot_base<Args...>(c, gid), pmf{std::forward<F>(f)}, ptr{std::forward<P>(p)} {}
...
class slot_pmf_extended final : public slot_base<Args...> {
   public:
    template <typename F, typename P>
    constexpr slot_pmf_extended(cleanable& c, F&& f, P&& p, group_id gid) : slot_base<Args...>(c, gid), pmf{std::forward<F>(f)}, ptr{std::forward<P>(p)} {}
...
class slot_tracked final : public slot_base<Args...> {
   public:
    template <typename F, typename P>
    constexpr slot_tracked(cleanable& c, F&& f, P&& p, group_id gid) : slot_base<Args...>(c, gid), func{std::forward<F>(f)}, ptr{std::forward<P>(p)} {}
...
class slot_pmf_tracked final : public slot_base<Args...> {
   public:
    template <typename F, typename P>
    constexpr slot_pmf_tracked(cleanable& c, F&& f, P&& p, group_id gid) : slot_base<Args...>(c, gid), pmf{std::forward<F>(f)}, ptr{std::forward<P>(p)} {}
...
class signal_base final : public detail::cleanable {
   ...
   public:
    ...
    /**
			 ...
			 */
    template <typename Pmf, typename Ptr>
    std::enable_if_t<trait::is_callable_v<arg_list, Pmf, Ptr> && !trait::is_observer_v<Ptr> && !trait::is_weak_ptr_compatible_v<Ptr>, connection> connect(
    ...
    /**
			 ...
			 */
    template <typename Callable>
    std::enable_if_t<(trait::is_callable_v<arg_list, Callable> || trait::is_callable_v<ext_arg_list, Callable> || trait::is_pmf_v<Callable>) &&
    ...
    std::enable_if_t<!trait::is_callable_v<arg_list, Obj> && !trait::is_callable_v<ext_arg_list, Obj> && !trait::is_pmf_v<Obj>, size_t> disconnect(
        const Obj& obj) {

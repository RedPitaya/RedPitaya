...
void setRegister(__attribute__((unused)) volatile OscilloscopeMapT* baseOsc_addr, volatile uint32_t* reg, int32_t value) {
    // acprintf(stderr, PColor::RED,"\tSet register 0x%X <- 0x%X\n",(uint32_t)reg-(uint32_t)baseOsc_addr,value);
    *reg = value;
}

auto COscilloscope::create(const UioT& _uio, uint32_t _dec_factor, bool _isMaster, uint32_t _adcMaxSpeed, bool _isADCFilterPresent, uint8_t _fpgaBits,
...
auto COscilloscope::setDataSize(uint32_t size) -> void {
    m_dataSize = size;
    setRegister(m_OscMap, &(m_OscMap->dma_buf_size), m_dataSize);

    // Trigger pre samples
    setRegister(m_OscMap, &(m_OscMap->trig_pre_samp), m_isMaster ? m_dataSize / 4 : 0);
    //_OscMap->trig_pre_samp = osc_buf_pre_samp;

    // Trigger post samples
    setRegister(m_OscMap, &(m_OscMap->trig_post_samp), m_dataSize / 4 * 3);
    // _OscMap->trig_post_samp = osc_buf_post_samp;
}

auto COscilloscope::setSkipDataAddress(uio_lib::MemoryRegionT block) -> void {
    setRegister(m_OscMap, &(m_OscMap->dma_dst_addr1_ch1), block.start);
    setRegister(m_OscMap, &(m_OscMap->dma_dst_addr1_ch2), block.start);
    setRegister(m_OscMap, &(m_OscMap->dma_dst_addr2_ch1), block.start);
    setRegister(m_OscMap, &(m_OscMap->dma_dst_addr2_ch2), block.start);
    if (m_maxChannels >= 3) {
        setRegister(m_OscMap, &(m_OscMap->dma_dst_addr1_ch3), block.start);
        setRegister(m_OscMap, &(m_OscMap->dma_dst_addr2_ch3), block.start);
    }
    if (m_maxChannels >= 4) {
        setRegister(m_OscMap, &(m_OscMap->dma_dst_addr1_ch4), block.start);
        setRegister(m_OscMap, &(m_OscMap->dma_dst_addr2_ch4), block.start);
    }
}

auto COscilloscope::setDataAddress(uint8_t index, uint32_t ch1, uint32_t ch2, uint32_t ch3, uint32_t ch4) -> void {
    if (index == 0) {
        if (ch1)
            setRegister(m_OscMap, &(m_OscMap->dma_dst_addr1_ch1), ch1);
        if (ch2)
            setRegister(m_OscMap, &(m_OscMap->dma_dst_addr1_ch2), ch2);
        if (m_maxChannels >= 3) {
            if (ch3)
                setRegister(m_OscMap, &(m_OscMap->dma_dst_addr1_ch3), ch3);
        }
        if (m_maxChannels >= 4) {
            if (ch4)
                setRegister(m_OscMap, &(m_OscMap->dma_dst_addr1_ch4), ch4);
        }
    }

    if (index == 1) {
        if (ch1)
            setRegister(m_OscMap, &(m_OscMap->dma_dst_addr2_ch1), ch1);
        if (ch2)
            setRegister(m_OscMap, &(m_OscMap->dma_dst_addr2_ch2), ch2);
        if (m_maxChannels >= 3) {
            if (ch3)
                setRegister(m_OscMap, &(m_OscMap->dma_dst_addr2_ch3), ch3);
        }
        if (m_maxChannels >= 4) {
            if (ch4)
                setRegister(m_OscMap, &(m_OscMap->dma_dst_addr2_ch4), ch4);
        }
    }
}
...
auto COscilloscope::clearBuffer(uint8_t index) -> bool {
    // clearInterrupt();

    uint32_t clearFlag = (index == 0 ? 0x00000004 : 0x00000008);  // reset buffer

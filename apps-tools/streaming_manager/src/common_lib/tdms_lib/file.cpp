...
auto File::GetMetadataItem(Reader &reader, shared_ptr<Segment> segment, map<string, map<string, shared_ptr<Metadata>>> &prevMetaDataLookup)
	...
	for (auto &metadata : metadataWithImplicit) {
		if (metadata->Path.size() == 2) {
			if (!key_exists<map<string, map<string, shared_ptr<Metadata>>>, string>(prevMetaDataLookup, metadata->Path[0])) {
				auto pair_data = std::pair<string, map<string, shared_ptr<Metadata>>>(metadata->Path[0],
																					  map<string, shared_ptr<Metadata>>());

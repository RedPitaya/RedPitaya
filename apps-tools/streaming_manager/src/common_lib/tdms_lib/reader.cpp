	...
	for (int32_t x = 0; x < objectCount; x++) {
		...
		if (rawDataIndexLength > 0) {
			...
			metadata->RawData.Size = rawDataIndexLength == 28 ? (long) m_bstream.Read<int64_t>(*m_fileStream, TDMSType::Integer64)
															  : (long) DataType::GetArrayLength(metadata->RawData.DataType.GetDataType(),
		...
		for (auto &metadata : metadatas) {
			...
			if (interleaveStride) {
				metadata->RawData.Count = segment->NextSegmentOffset > 0
											  ? (segment->NextSegmentOffset - metadata->RawData.Offset + interleaveStride - 1)
													/ interleaveStride
...
auto Reader::ReadRawData(RawData &rawData) -> vector<shared_ptr<DataType::Raw>>
{
	if (rawData.IsInterleaved)
		return ReadRawInterleaved(rawData.Offset,
								  rawData.Count,
								  rawData.DataType.GetDataType(),

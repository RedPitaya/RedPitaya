...
auto createPlatformOscilloscope(const std::vector<UioT> &uioList,
								uint32_t rate,
								bool is_master,
								bool filterBypass,
								...
								uint8_t resolution_bits) -> COscilloscope::Ptr
{
	...
	for (const auto &uio : uioList) {
		if (uio.nodeName == "rp_oscilloscope@40000000") {
			...
			auto osc =
				COscilloscope::create(uio, rate, is_master, ClientOpt::getADCRate(), !filterBypass, ClientOpt::getADCBits(), max_channels);
...
auto initializeMemory(const CStreamSettings &settings, uint8_t channelsActive, bool buffersTestMode) -> bool
{
	...
	if (g_verbMode) {
		printWithLog(LOG_INFO,
					 stdout,
					 "Reserved memory blocks: %d size: %d\n",
					 reservedBlocks,
					 reservedBlocks * memmanager->getMemoryBlockSize());
	}
...
auto startServer(bool verbMode, bool testMode, bool is_master) -> void
{
	...
	try {
		...
		if (use_file.value == CStreamSettings::PassMode::FILE) {
			...
			g_s_file = streaming_lib::CStreamingFile::create(settings.getADCFormat(),
															 path,
															 settings.getADCSamples(),
															 settings.getADCType().value == CStreamSettings::DataType::VOLT,
															 testMode,
															 is_platform);
		...
		// Generate buffers
		auto memmanager = uio_lib::CMemoryManager::instance();
		g_s_buffer->generateBuffers(memmanager->getRegions(MM_ADC),
									(use_file.value == CStreamSettings::PassMode::NET ? DataLib::sizeHeader() : 0),
									buffersTestMode);

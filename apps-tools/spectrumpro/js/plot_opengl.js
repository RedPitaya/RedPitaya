...
class GLMode {
    ...
    drawSeriesLines(gl, programInfo, series, points_x, points_y) {
        ...
        gl.uniform4fv(programInfo.u_color, this.hexToFloatArray(series.data.color));
        gl.lineWidth(1.0);
    ...
    drawSeries(plot, ctx, series) {
        ...
        if (series.data && series.data.data_gl_x != undefined && series.data.data_gl_y != undefined && dpi && dgl) {
            if (series.data.lines.show_gl || series.data.points.show_gl) {
                let l = series.data.data_gl_x.length
                if (dpi.indices == undefined) {
                ...
                for (let i = 0; i < l; i++) {
                    const x = series.data.data_gl_x[i];
                    const y = series.data.data_gl_y[i];
                ...
                if (series.data.lines.show_gl) {
                    this.drawSeriesLines(dgl, dpi, series, dpi.indices, dpi.y_values);
                }

                ctx.drawImage(dc, offset.left, offset.top);
            }
        }
    }
}
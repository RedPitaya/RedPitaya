...
Write-Host "Step 1: Running Makefile inside Red Pitaya OS container..." -ForegroundColor Cyan
docker run --rm `
...
Write-Host "Step 2: Running Makefile.x86 setup..." -ForegroundColor Cyan
docker run --rm `
    -v "${PROJECT_ROOT}:/workspace" `
    -w /workspace `
    $BUILD_ENV_IMG `

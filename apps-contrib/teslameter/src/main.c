...
pthread_mutex_t rp_main_params_mutex = PTHREAD_MUTEX_INITIALIZER; //api
static rp_app_params_t rp_main_params[PARAMS_NUM+1] = {
    ...
    { /* forcex_flag: 
       ...
       **/
    {  "meas_min_ch1", 0, 0, 1, -100000, +100000 },
    {  "meas_max_ch1", 0, 0, 1, +100000, -100000 },
    {  "meas_amp_ch1", 0, 0, 1, +100000, -100000 },
    {  "meas_avg_ch1", 0, 0, 1, +1000, -1000 },
    {  "meas_freq_ch1", 0, 0, 1, 0, 1e9 },
    {  "meas_per_ch1", 0, 0, 1, 0, 1e9 },
    {  "meas_min_ch2", 0, 0, 1, -100000, +100000 },
    {  "meas_max_ch2", 0, 0, 1, +100000, -100000 },
    {  "meas_amp_ch2", 0, 0, 1, +100000, -100000 },
    ...
    { /* teslameter scale - this parameter is set by user and is multiplys the signal*/
        "scale_tesla_ch1", 1, 0, 0, 0, 1000000000 },
    { /* teslameter scale - this parameter is set by user and is multiplys the signal*/
        "scale_tesla_ch2", 1, 0, 0, 0, 1000000000 },
    { /* teslameter scale - this parameter is set by user and is multiplys the signal*/
        "scale_decade_tesla_ch1", 0, 0, 0, 0, 2 },
    { /* teslameter scale - this parameter is set by user and is multiplys the signal*/
        "scale_decade_tesla_ch2", 0, 0, 0, 0, 2 },
...
int rp_update_meas_data(rp_osc_meas_res_t ch1_meas, rp_osc_meas_res_t ch2_meas, int tesla_fd)
{
    ...
    float amplitude = ch2_meas.amp;


        power_led(1, tesla_fd); // tur on leds
        ...
        if ( amplitude > 0.1){
          ...
          power_led(2, tesla_fd); // tur on leds
        }
        else if ( amplitude > 0.5){

          power_led(3, tesla_fd); // tur on leds
        }
        else if ( amplitude > 0.7){

          power_led(4, tesla_fd); // tur on leds
        }
        else if ( amplitude > 0.8){

          power_led(5, tesla_fd); // tur on leds
        }
        else if ( amplitude > 0.9){

          power_led(6, tesla_fd); // tur on leds
        }
        else if ( amplitude > 1){

          power_led(7, tesla_fd); // tur on leds
        }
        else{
           power_led(7, tesla_fd); // tur on leds
      }

...
template <typename T>
int analysisTrap(const T* ch1, const T* ch2, const size_t ch1_size, const size_t ch2_size, uint32_t signalFreq, uint32_t decimation, uint32_t adcRate, T input_threshold,
                 T* phaseCh1, T* phaseCh2, T* gain, T* phaseDiff) {
    constexpr size_t ADC_BUFFER_SIZE = 1024 * 16;
    if (adcRate == 0)
        return RP_A_ERROR;
    if (ch1_size != ch2_size)
        return RP_A_ERROR;
    if (ch1_size == 0 || ch2_size == 0)
        return RP_A_DATA_SIZE_ZERO;
    if (ch1_size > ADC_BUFFER_SIZE)
        return RP_A_ERROR;
...
int analysisTrap(const std::vector<float>& ch1, const std::vector<float>& ch2, uint32_t signalFreq, uint32_t decimation, uint32_t adcRate, float input_threshold, float* phaseCh1,
                 float* phaseCh2, float* gain, float* phaseDiff) {
...
int analysisTrap(const std::vector<double>& ch1, const std::vector<double>& ch2, uint32_t signalFreq, uint32_t decimation, uint32_t adcRate, float input_threshold,
...
int analysisTrap(const float* ch1, const float* ch2, size_t ch1_size, size_t ch2_size, uint32_t signalFreq, uint32_t decimation, uint32_t adcRate, float input_threshold,
...
int analysisTrap(const double* ch1, const double* ch2, size_t ch1_size, size_t ch2_size, uint32_t signalFreq, uint32_t decimation, uint32_t adcRate, float input_threshold,
                 double* phaseCh1, double* phaseCh2, double* gain, double* phaseDiff) {
...
int analysis(std::vector<float>& ch1, std::vector<float>& ch2, uint32_t signalFreq, uint32_t periods, uint32_t decimation, uint32_t adcRate, float* amplutudeCh1,
             float* amplutudeCh2, float* phaseCh1, float* phaseCh2, float* gain, float* phaseDiff) {
    float ampl[2];
    float phase[2];
...
int analysis(std::vector<double>& ch1, std::vector<double>& ch2, uint32_t signalFreq, uint32_t periods, uint32_t decimation, uint32_t adcRate, double* amplutudeCh1,
             double* amplutudeCh2, double* phaseCh1, double* phaseCh2, double* gain, double* phaseDiff) {
    double ampl[2];
    double phase[2];
...
int analysis(float* ch1, float* ch2, size_t ch1_size, size_t ch2_size, uint32_t signalFreq, uint32_t periods, uint32_t decimation, uint32_t adcRate, float* amplutudeCh1,
             float* amplutudeCh2, float* phaseCh1, float* phaseCh2, float* gain, float* phaseDiff) {
    float ampl[2];
    float phase[2];
...
int analysis(double* ch1, double* ch2, size_t ch1_size, size_t ch2_size, uint32_t signalFreq, uint32_t periods, uint32_t decimation, uint32_t adcRate, double* amplutudeCh1,
             double* amplutudeCh2, double* phaseCh1, double* phaseCh2, double* gain, double* phaseDiff) {
    double ampl[2];
    double phase[2];
...
template <typename T>
int analysisFFT(const T* ch1, const T* ch2, size_t size, T _freq, int decimation, T* gain, T* phase_out, float input_threshold) {
    std::lock_guard lock(g_fft_mutex);
...
int analysisFFT(const float* ch1, const float* ch2, size_t size, float freq, int decimation, float* gain, float* phase_out, float input_threshold) {
    return analysisFFT<float>(ch1, ch2, size, freq, decimation, gain, phase_out, input_threshold);
}

int analysisFFT(const double* ch1, const double* ch2, size_t size, double freq, int decimation, double* gain, double* phase_out, float input_threshold) {
    return analysisFFT<double>(ch1, ch2, size, freq, decimation, gain, phase_out, input_threshold);
}

int analysisFFT(const std::vector<float>& ch1, const std::vector<float>& ch2, float freq, int decimation, float* gain, float* phase_out, float input_threshold) {
...
int analysisFFT(const std::vector<double>& ch1, const std::vector<double>& ch2, double freq, int decimation, double* gain, double* phase_out, double input_threshold) {
    if (ch1.size() != ch2.size())
        return RP_A_ERROR;
    auto size = ch1.size();

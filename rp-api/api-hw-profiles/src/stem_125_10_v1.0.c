...
profiles_t stem_125_10_v1_0 = {
    ...
    .fast_adc_bits = 14, // Must be 10, but fpga return value in 14 bit
...
profiles_t* getProfile_STEM_125_10_v1_0(){
    ...
    if (!initialized) {
        fast_adc_rate = stem_125_10_v1_0.fast_adc_rate;
        fast_dac_rate = stem_125_10_v1_0.fast_dac_rate;
        initialized = true;
    }

    int rate = hp_cmn_GetADCBaseRateFromConfig(stem_125_10_v1_0.boardModel);
    if (rate != 0){
        stem_125_10_v1_0.fast_adc_rate = rate;
    }else{
        stem_125_10_v1_0.fast_adc_rate = fast_adc_rate;
    }

    rate = hp_cmn_GetDACBaseRateFromConfig(stem_125_10_v1_0.boardModel);
    if (rate != 0){
        stem_125_10_v1_0.fast_dac_rate = rate;
    }else{
        stem_125_10_v1_0.fast_dac_rate = fast_dac_rate;
    }

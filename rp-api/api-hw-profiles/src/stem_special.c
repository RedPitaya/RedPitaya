...
profiles_t* getProfile_STEM_SPECIAL(){
    ...
    if (!initialized) {
        fast_adc_rate = stem_special.fast_adc_rate;
        fast_dac_rate = stem_special.fast_dac_rate;
        initialized = true;
    }

    int rate = hp_cmn_GetADCBaseRateFromConfig(stem_special.boardModel);
    if (rate != 0){
        stem_special.fast_adc_rate = rate;
    }else{
        stem_special.fast_adc_rate = fast_adc_rate;
    }

    rate = hp_cmn_GetDACBaseRateFromConfig(stem_special.boardModel);
    if (rate != 0){
        stem_special.fast_dac_rate = rate;
    }else{
        stem_special.fast_dac_rate = fast_dac_rate;
    }

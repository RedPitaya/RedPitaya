#include "stem_122_16SDR_v1.0.h"
#include "common.h"

profiles_t stem_122_16SDR_v1_0 = {
    .boardModel = STEM_122_16SDR_v1_0,
    .boardName = "SDRlab 122-16 v1.0",
    .boardModelEEPROM = "",
    .boardETH_MAC =  "",
    .zynqCPUModel = Z7020,
    .oscillator_rate = 122880000,

    .fast_adc_rate = 122880000,
    .fast_adc_count_channels = 2,
    .fast_adc_is_sign = true,
    .fast_adc_bits = 16,

    .fast_adc_gain = {
        [RP_HP_ADC_GAIN_NORMAL] = {0.5, 0.5, 0, 0},
        [RP_HP_ADC_GAIN_HIGH] = {0, 0, 0, 0}
    },

    .is_dac_present = true,
    .fast_dac_rate = 122880000,
    .fast_dac_count_channels = 2,
    .fast_dac_is_sign = true,
    .fast_dac_bits = 14,
    .fast_dac_out_full_scale = {0.5, 0.5, 0, 0},

    .is_LV_HV_mode = false,
    .is_AC_DC_mode = false,

    .slow_adc_count_channels = 4,
    // Real 12 bits, but we need to exclude the sign
    .slow_adc = {
        [0] = { .is_signed = false, .bits = 11, .fullScale = 3.5f },
        [1] = { .is_signed = false, .bits = 11, .fullScale = 3.5f },
        [2] = { .is_signed = false, .bits = 11, .fullScale = 3.5f },
        [3] = { .is_signed = false, .bits = 11, .fullScale = 3.5f }
    },

    .slow_dac_count_channels = 4,
    .slow_dac = {
        [0] = { .is_signed = false, .bits = 8, .fullScale = 1.8f },
        [1] = { .is_signed = false, .bits = 8, .fullScale = 1.8f },
        [2] = { .is_signed = false, .bits = 8, .fullScale = 1.8f },
        [3] = { .is_signed = false, .bits = 8, .fullScale = 1.8f }
    },

    .is_DAC_gain_x5 = false,
    .is_fast_calibration = false,
    .is_pll_control_present = false,
    .is_fast_adc_filter_present = false,
    .is_fast_dac_temp_protection = false,
    .is_attenuator_controller_present = false,
    .is_ext_trigger_level_available = false,
    .external_trigger_full_scale = 0,
    .is_ext_trigger_signed = false,
    .fast_adc_spectrum_resolution = 61440000,
    .fast_adc_full_scale = 1,
    .fast_dac_full_scale = 1,
    .is_daisy_chain_clock_sync = false,
    .is_dma_mode_v0_94 = true,
    .is_DAC_50_Ohm_mode = false,
    .is_split_osc_triggers = true,
    .gpio_N_count = 11,
    .gpio_P_count = 11,
    .ramMB = 512,
    .is_E3_high_speed_gpio = false,
    .is_E3_mmc_qspi = false,
    .E3_high_speed_gpio_rate = 0,
    .is_E3_present = false,
    .is_calib_in_fpga = false
};

profiles_t* getProfile_STEM_122_16SDR_v1_0(){

    static uint32_t fast_adc_rate = 0;
    static uint32_t fast_dac_rate = 0;

    static bool initialized = false;
    if (!initialized) {
        fast_adc_rate = stem_122_16SDR_v1_0.fast_adc_rate;
        fast_dac_rate = stem_122_16SDR_v1_0.fast_dac_rate;
        initialized = true;
    }

    int rate = hp_cmn_GetADCBaseRateFromConfig(stem_122_16SDR_v1_0.boardModel);
    if (rate != 0){
        stem_122_16SDR_v1_0.fast_adc_rate = rate;
    }else{
        stem_122_16SDR_v1_0.fast_adc_rate = fast_adc_rate;
    }

    rate = hp_cmn_GetDACBaseRateFromConfig(stem_122_16SDR_v1_0.boardModel);
    if (rate != 0){
        stem_122_16SDR_v1_0.fast_dac_rate = rate;
    }else{
        stem_122_16SDR_v1_0.fast_dac_rate = fast_dac_rate;
    }

    return &stem_122_16SDR_v1_0;
}

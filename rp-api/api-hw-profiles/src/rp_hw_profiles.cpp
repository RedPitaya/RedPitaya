...
int rp_HPGetFastADCMaxLowPassFilterHz(uint32_t* value) {
    int state;
    profiles_t* p = getProfile(&state);
    if (p) {
        *value = p->fast_adc_low_pass_filter;
        return RP_HP_OK;
    }
    return state;
}
...
uint32_t rp_HPGetFastADCMaxLowPassFilterHzOrDefault() {
    profiles_t* p = getProfileDefualt();
    return p->fast_adc_low_pass_filter;
}

int rp_HPGetFastDACMaxLowPassFilterHz(uint32_t* value) {
    int state;
    profiles_t* p = getProfile(&state);
    if (p) {
        *value = p->fast_dac_low_pass_filter;
        return RP_HP_OK;
    }
    return state;
}
...
uint32_t rp_HPGetFastDACMaxLowPassFilterHzOrDefault() {
    profiles_t* p = getProfileDefualt();
    return p->fast_dac_low_pass_filter;
}

int rp_HPGetGenMinSpeedHz(uint32_t* value) {
    int state;
    profiles_t* p = getProfile(&state);
    if (p) {
        *value = p->gen_min_speed;
        return RP_HP_OK;
    }
    return state;
}
...
uint32_t rp_HPGetGenMinSpeedHzOrDefault() {
    profiles_t* p = getProfileDefualt();
    return p->gen_min_speed;
}

int rp_HPGetGenMaxSpeedHz(uint32_t* value) {
    int state;
    profiles_t* p = getProfile(&state);
    if (p) {
        *value = p->gen_max_speed;
        return RP_HP_OK;
    }
    return state;
}
...
int rp_HPWriteUserDefinedValue(const char* key, int value) {
    ...
    if (ret == RP_HP_OK) {
        ret = hp_cmn_WriteConfig(model, key, value);
    }
    return ret;
}

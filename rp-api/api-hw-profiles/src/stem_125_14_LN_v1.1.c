...
profiles_t* getProfile_STEM_125_14_LN_v1_1(){
    ...
    if (!initialized) {
        fast_adc_rate = stem_125_14_LN_v1_1.fast_adc_rate;
        fast_dac_rate = stem_125_14_LN_v1_1.fast_dac_rate;
        initialized = true;
    }

    int rate = hp_cmn_GetADCBaseRateFromConfig(stem_125_14_LN_v1_1.boardModel);
    if (rate != 0){
        stem_125_14_LN_v1_1.fast_adc_rate = rate;
    }else{
        stem_125_14_LN_v1_1.fast_adc_rate = fast_adc_rate;
    }

    rate = hp_cmn_GetDACBaseRateFromConfig(stem_125_14_LN_v1_1.boardModel);
    if (rate != 0){
        stem_125_14_LN_v1_1.fast_dac_rate = rate;
    }else{
        stem_125_14_LN_v1_1.fast_dac_rate = fast_dac_rate;
    }

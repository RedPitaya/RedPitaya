...
int rp_Run(uint32_t noOfPreTriggerSamples,
    ...
    double timeIntervalNanoseconds;
    uint32_t maxSamples = rp_LaAcqBufLenInSamples(&g_la_acq_handle);
...
int rp_WaitData(int timeout){
    ...
    int ret = rp_LaAcqRead(&g_la_acq_handle, timeout);
    ...
    if(rle){
        ...
        uint32_t current;
        bool buf_ovfl;
        rp_LaAcqGetRLEStatus(&g_la_acq_handle, &current, &last_sample, &buf_ovfl);
        trig_sample = 0;
    }
    else{
    ...
    g_acq_data.trig_sample=trig_sample;
    g_acq_data.last_sample=last_sample;
...
int rp_GetTrigPosition(uint32_t * tigger_pos){
    std::lock_guard lock(g_data_mtx);
	*tigger_pos = g_acq_data.trig_sample;
	return RP_OK;
}
...
int rp_GetValues(uint32_t *noOfSamples){
    ...
    if(rle){ // RLE mode
        ...
        while(1){
            ...
            if (index == 0){
                index = rp_LaAcqBufLenInSamples(&g_la_acq_handle) - 1;
            }
            else{
                index--;
            }
        ...
        index = first_sample;
        for( i = 0; i < samples; i++){
            if (index >= rp_LaAcqBufLenInSamples(&g_la_acq_handle)){
                index = 0;
            }
            g_acq_data.buf[i] = map[index];
            // adjust length of first sample
            if (i == 0){
                g_acq_data.buf[i] -= (int16_t)(fist_sample_len_adj << 8);
            }
            index++;
        }
        g_acq_data.trig_sample = samples - g_acq_data.trig_sample;
        *noOfSamples = samples;
    }
    else{
        int32_t first_sample = g_acq_data.trig_sample - g_acq_data.pre_samples;
        int32_t last_sample = g_acq_data.trig_sample + g_acq_data.post_samples;
        int32_t buf_len = rp_LaAcqBufLenInSamples(&g_la_acq_handle);

        TRACE_SHORT("req: pre=%d pos=%d", g_acq_data.pre_samples, g_acq_data.post_samples);
        TRACE_SHORT("sta: first=%d trig=%d last=%d", first_sample, g_acq_data.trig_sample, last_sample);

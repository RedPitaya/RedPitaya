...
auto CANParameters::fromJson(const std::string& json) -> bool {
    ...
    try {
        ...
        if (!parseUInt32(m_fast_bitrate, "fast_bitrate"))
            return false;
        if (!parseFloat(m_sample_point, "sample_point"))
            return false;
...
std::string CANParameters::getCANAnnotationsString(CANAnnotations value) {
    switch (value) {
        ...
        case IDE:
            return "Identifier extension bit";
        case RESERV_BIT:
            return "Reserved bit 0 and 1";

        case RTR:
            return "Remote transmission request";
        case SRR:
            return "Substitute remote request";
        case DLC:
        ...
        case ERROR_3:
            return "End of frame (EOF) must be 7 recessive bits";

        case WARNING_1:
            return "Identifier bits 10..4 must not be all recessive";
        case WARNING_2:
            return "CRC delimiter must be a recessive bit";
        case WARNING_3:
            return "ACK delimiter must be a recessive bit";

        case BRS:
            return "Bit rate switch";
        case ESI:
            return "Error state indicator";
        case RESERV_BIT_FLEX:
            return "Flexible data";
        case STUFF_BIT_ERROR:

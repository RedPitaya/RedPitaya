...
class SPIDecoder::Impl {
    ...
    enum State { FIND_DATA, COLLECT_DATA };
   ...
   public:
    ...
    uint32_t m_clockWidth = 0;
    uint32_t m_clockChangeSample = 0;
...
bool SPIDecoder::Impl::findClkEdge(bool data, bool clk, bool cs) {
    ...
    if (clk == m_oldclk)
        return false;
    m_clockWidth = m_samplenum - m_clockChangeSample;
    m_clockChangeSample = m_samplenum;
    m_oldclk = clk;
...
void SPIDecoder::Impl::handleBit(bool data, bool clk, bool cs) {
    ...
    auto data_len = m_samplenum - m_startSamplenum + m_clockWidth * 2;

...
auto UARTParameters::fromJson(const std::string& json) -> bool {
    ...
    try {
        ...
        if (!parseUInt32(m_rx, "rx"))
            return false;
        if (!parseUInt32(m_tx, "tx"))
            return false;
        if (!parseUInt32(m_baudrate, "baudrate"))
            return false;
        if (!parseUInt32(m_invert, "invert"))
            return false;

        uint32_t x;
        if (!parseUInt32(x, "bitOrder"))
            return false;
        CHECK_ENUM(g_eBitOrderArray, x)
        m_bitOrder = (UartBitOrder)x;

        if (!parseUInt32(x, "num_data_bits"))
            return false;
        CHECK_ENUM(g_eNumBitArray, x)
        m_num_data_bits = (NumDataBits)x;

        if (!parseUInt32(x, "parity"))
            return false;
        CHECK_ENUM(g_eParityArray, x)
        m_parity = (Parity)x;

        if (!parseUInt32(x, "num_stop_bits"))
            return false;

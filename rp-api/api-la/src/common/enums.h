#ifndef LA_LIB_ENUMS_H
#define LA_LIB_ENUMS_H
...
#define EVALUATE_COUNT(_1, _2, _3, _4, _5, _6, _7, _8, _9, count, ...) count

#define COUNT(...) IDENTITY(EVALUATE_COUNT(__VA_ARGS__, 9, 8, 7, 6, 5, 4, 3, 2, 1))
    ...
    struct EnumName {                                                                                      \
        ...
        static _enumerated from_string(const std::string& name) noexcept(false) {                          \
            ...
            throw std::invalid_argument("Not found");                                                      \
                                                                                                           ...
                                                                                                           \
        static _enumerated from_int(const int value) noexcept(false) {                                     \
            for (size_t index = 0; index < count; ++index) {                                               \
                if (values()[index] == value)                                                              \
                    return (_enumerated)values()[index];                                                   \
            }                                                                                              \
            ...
            if (!initialized) {                                                                            \
                for (size_t index = 0; index < count; ++index) {                                           \
                    ...
                    processed_names[index] = new char[length + 1];                                         \
                                                                                                           \
                    std::memcpy(processed_names[index], raw_names[index], length);                         \
        ...
        static const char* const* strings() {                                                              \
            static const char* const raw_names[] = {IDENTITY(EXTRACT_EACH_SECOND(__VA_ARGS__))};           \
                                                                                                           \
            static char* processed_names[count];                                                           \
            static bool initialized = false;                                                               \
                                                                                                           \

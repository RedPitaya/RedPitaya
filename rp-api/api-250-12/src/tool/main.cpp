...
void UsingArgs(char const* progName) {
    ...
    printf("\t-C Compare register values with configuration file.\n\t   If the values match, then the return result is 0.\n");
...
int main(int argc, char* argv[]) {
    ...
    if (read_value) {
        ...
        if (CheckMissing(I2C_DEVICE, "I2C_DEVICE") || CheckMissing(ADDRESS, "ADDRESS") || CheckMissing(REGISTER, "REGISTER")) {
            UsingArgs(argv[0]);
        }
        int addr = 0;
        int reg = 0;
    ...
    if (write_value) {
        ...
        if (CheckMissing(I2C_DEVICE, "I2C_DEVICE") || CheckMissing(ADDRESS, "ADDRESS") || CheckMissing(VALUE, "VALUE") || CheckMissing(REGISTER, "REGISTER")) {
            UsingArgs(argv[0]);
        }
        int addr = 0;
        int reg = 0;

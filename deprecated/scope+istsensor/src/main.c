...
static rp_app_params_t rp_main_params[PARAMS_NUM+1] = {
    ...
    { /* max_gui_time   */ 
        ...
        "xmax", 100, 1, 0, -10000000, +10000000 },
    { /* trig_mode:
       *    0 - auto
       *    1 - normal
       *    2 - single  */
    ...
    {  "IST_min", 0, 0, 1, -1000, +1000 },
    {  "IST_max", 0, 0, 1, +1000, -1000 },
    {  "IST_adj", 0, 0, 1, +1000, -1000 },
    {  "IST_lm35", 0, 0, 1, +1000, -1000 },
    {  "IST_freq", 0, 0, 1, 0, 1e9 },
    {  "IST_per", 0, 0, 1, 0, 1e9 },
    ...
    { /* gui_xmax - Xmax as specified by GUI - not rounded to sampling engine quanta. */
        "gui_xmax",    100, 0, 1, -10000000, +10000000 },
    ...
    { /* pid_IST_enable - Enables/closes or disables/open PID loop for IST Sensor:
       *    0 - IST RealProbe PID disabled (open loop)
       *    1 - IST RealProbe PID enabled (closed loop)    */
        "pid_IST_enable", 0, 1, 0, 0, 1 },
    { 
        "IST_save2file", 0, 1, 0, 0, 1 },
    { /* the Delta Temperature between the LM35 and IST sensor */
        "pid_IST_DT",  0, 1, 0, -8192, 8191 },
    { /* IST PID proportional gain Kp in [ADC] counts. */
        "pid_IST_kp",  0, 1, 0, -8192, 8191 },
    { /* IST PID integral gain     Ki in [ADC] counts. */
        "pid_IST_ki",  0, 1, 0, -8192, 8191 },
    { /* IST PID derivative gain   Kd in [ADC] counts. */
...
int rp_update_meas_data(rp_osc_meas_res_t ch1_meas, rp_osc_meas_res_t ch2_meas)
{
    ...
    rp_main_params[IST_MIN].value = ISTmin;
    rp_main_params[IST_MAX].value = ISTmax;
    rp_main_params[IST_ADJ].value = ISTadj;
    rp_main_params[IST_LM35].value = ISTlm35;
    rp_main_params[IST_FREQ].value = ISTfreq;
    rp_main_params[IST_PER].value = ISTper;

    rp_main_params[PID_IST_DT].value = DeltaTemp;
    rp_main_params[PID_IST_KP].value = kp;
    rp_main_params[PID_IST_KI].value = ki;
    rp_main_params[PID_IST_KD].value = kd;

 ...
 */
static int __pwr_fpga_cleanup_mem(void)
{
    /* optionally unmap memory regions  */
    if (g_pwr_fpga_reg_mem) {
        if (munmap(g_pwr_fpga_reg_mem, PWR_FPGA_BASE_SIZE) < 0) {
            fprintf(stderr, "munmap() failed: %s\n", strerror(errno));
            return -1;
        }
        /* ...and update memory pointers */
        g_pwr_fpga_reg_mem = NULL;
        g_pwr_fpga_cha_mem = NULL;
        g_pwr_fpga_chb_mem = NULL;
    }

    /* optionally close file descriptor */
    if(g_pwr_fpga_mem_fd >= 0) {
        close(g_pwr_fpga_mem_fd);
        g_pwr_fpga_mem_fd = -1;
    }
...
/*----------------------------------------------------------------------------*/
/**
 * @brief Initialize interface to Oscillpwrope FPGA module
 *
 * Function first optionally cleanups previously established access to Oscillpwrope
 * FPGA module. Afterwards a new connection to the Memory handler is instantiated
 * by opening file descriptor over /dev/mem device. Access to Oscillpwrope FPGA module
...
int pwr_fpga_init(void)
{
    ...
    if(__pwr_fpga_cleanup_mem() < 0)
        return -1;

    g_pwr_fpga_mem_fd = open("/dev/mem", O_RDWR | O_SYNC);
    if(g_pwr_fpga_mem_fd < 0) {
        fprintf(stderr, "open(/dev/mem) failed: %s\n", strerror(errno));
        return -1;
    }

    page_addr = PWR_FPGA_BASE_ADDR & (~(page_size-1));
    page_off  = PWR_FPGA_BASE_ADDR - page_addr;

    page_ptr = mmap(NULL, PWR_FPGA_BASE_SIZE, PROT_READ | PROT_WRITE,
                          MAP_SHARED, g_pwr_fpga_mem_fd, page_addr);
    if((void *)page_ptr == MAP_FAILED) {
        fprintf(stderr, "mmap() failed: %s\n", strerror(errno));
        __pwr_fpga_cleanup_mem();
        return -1;
    }
    g_pwr_fpga_reg_mem = page_ptr + page_off;
    g_pwr_fpga_cha_mem = (uint32_t *)g_pwr_fpga_reg_mem + 
        (PWR_FPGA_CHA_OFFSET / sizeof(uint32_t));
    g_pwr_fpga_chb_mem = (uint32_t *)g_pwr_fpga_reg_mem + 
        (PWR_FPGA_CHB_OFFSET / sizeof(uint32_t));
...
/*----------------------------------------------------------------------------*/
/**
 * @brief Setup Oscillpwrope FPGA module based on specified settings
 * FPGA module
 *
                           ...
                           int enable_avg_at_dec)
{
    ...
    int fpga_trig_source = pwr_fpga_cnv_trig_source(trig_imm, trig_source, 
                                                    trig_edge);
    int fpga_dec_factor = pwr_fpga_cnv_time_range_to_dec(time_range);
    int fpga_delay;
    ...
    if(trig_source == 0) {
        fpga_trig_thr = pwr_fpga_cnv_v_to_cnt(trig_level, ch1_adc_max_v,
        ...
        fpga_trig_thr = pwr_fpga_cnv_a_to_cnt(trig_level, ch2_adc_max_v,
    ...
    if((fpga_trig_source < 0) || (fpga_dec_factor < 0)) {
        fprintf(stderr, "pwr_fpga_update_params() failed\n");
        return -1;
    }
    ...
    after_trigger = 
        ((PWR_FPGA_SIG_LEN-7) * c_pwr_fpga_smpl_period * fpga_dec_factor) +
        trig_delay;

    if(after_trigger < 0)
        after_trigger = 0;

    fpga_delay = pwr_fpga_cnv_time_to_smpls(after_trigger, fpga_dec_factor);

    /* Trig source is written after ARM */
    /*    g_pwr_fpga_reg_mem->trig_source   = fpga_trig_source;*/
    if(trig_source == 0) 
        g_pwr_fpga_reg_mem->cha_thr   = fpga_trig_thr;
    else
        g_pwr_fpga_reg_mem->chb_thr   = fpga_trig_thr;
    g_pwr_fpga_reg_mem->data_dec      = fpga_dec_factor;
    g_pwr_fpga_reg_mem->trigger_delay = (uint32_t)fpga_delay;

    g_pwr_fpga_reg_mem->other = enable_avg_at_dec;
    
    
    // Updating hysteresys registers
    
    
    g_pwr_fpga_reg_mem->cha_hystersis=PWR_HYSTERESIS;
    g_pwr_fpga_reg_mem->chb_hystersis=PWR_HYSTERESIS;
    
    
    // Updating equalization filter with default coefficients
    if (ch1_gain==0)
    {
     g_pwr_fpga_reg_mem->cha_filt_aa =gain_hi_cha_filt_aa;
     g_pwr_fpga_reg_mem->cha_filt_bb =gain_hi_cha_filt_bb;
     g_pwr_fpga_reg_mem->cha_filt_pp =gain_hi_cha_filt_pp;
     g_pwr_fpga_reg_mem->cha_filt_kk =gain_hi_cha_filt_kk;
    }
    else
    {
     g_pwr_fpga_reg_mem->cha_filt_aa =gain_lo_cha_filt_aa;
     g_pwr_fpga_reg_mem->cha_filt_bb =gain_lo_cha_filt_bb;
     g_pwr_fpga_reg_mem->cha_filt_pp =gain_lo_cha_filt_pp;
     g_pwr_fpga_reg_mem->cha_filt_kk =gain_lo_cha_filt_kk;      
    }
    
       if (ch2_gain==0)
    {
     g_pwr_fpga_reg_mem->chb_filt_aa =gain_hi_chb_filt_aa;
     g_pwr_fpga_reg_mem->chb_filt_bb =gain_hi_chb_filt_bb;
     g_pwr_fpga_reg_mem->chb_filt_pp =gain_hi_chb_filt_pp;
     g_pwr_fpga_reg_mem->chb_filt_kk =gain_hi_chb_filt_kk;
    }
    else
    {
     g_pwr_fpga_reg_mem->chb_filt_aa =gain_lo_chb_filt_aa;
     g_pwr_fpga_reg_mem->chb_filt_bb =gain_lo_chb_filt_bb;
     g_pwr_fpga_reg_mem->chb_filt_pp =gain_lo_chb_filt_pp;
     g_pwr_fpga_reg_mem->chb_filt_kk =gain_lo_chb_filt_kk;      
    }
...
{
    g_pwr_fpga_reg_mem->conf |= PWR_FPGA_CONF_RST_BIT;
    return 0;
}
...
{
    g_pwr_fpga_reg_mem->conf |= PWR_FPGA_CONF_ARM_BIT;
...
{
    g_pwr_fpga_reg_mem->trig_source = trig_source;
    return 0;
}
...
{
    g_pwr_fpga_reg_mem->trigger_delay = trig_delay;
    return 0;
}
...
{
    return ((g_pwr_fpga_reg_mem->trig_source & PWR_FPGA_TRIG_SRC_MASK)==0);
}
...
{
    *cha_signal = (int *)g_pwr_fpga_cha_mem;
    *chb_signal = (int *)g_pwr_fpga_chb_mem;
    return 0;
}
...
{
    if(wr_ptr_curr)
        *wr_ptr_curr = g_pwr_fpga_reg_mem->wr_ptr_cur;
    if(wr_ptr_trig)
        *wr_ptr_trig = g_pwr_fpga_reg_mem->wr_ptr_trigger;
    return 0;
}
...
int pwr_fpga_cnv_time_to_smpls(float time, int dec_factor)
{
    ...
    float smpl_p = (c_pwr_fpga_smpl_period * dec_factor);
 ...
 * DC offsets.
 *
 * @param[in] u_voltage      Voltage in user units, specified in [V]
 ...
 * @param[in] diff_probe_att Differential probe attenuation
 * @retval    int            ADC counts
 */
int pwr_fpga_cnv_v_to_cnt(float u_voltage, float adc_max_v,
                          int calib_dc_off, float user_dc_off,
                          float diff_probe_att)
{
    int adc_cnts = 0;
    float voltage = 0; //voltage on input
    
    voltage = u_voltage / diff_probe_att;
    ...
    adc_cnts = (int)round(voltage * (float)((int)(1<<c_pwr_fpga_adc_bits)) / 
                          (2*adc_max_v));

    /* clip to the highest value (we are dealing with 14 bits only) */
    if((voltage > 0) && (adc_cnts & (1<<(c_pwr_fpga_adc_bits-1))))
        adc_cnts = (1<<(c_pwr_fpga_adc_bits-1))-1;
    else
        adc_cnts = adc_cnts & ((1<<(c_pwr_fpga_adc_bits))-1);
...
/*----------------------------------------------------------------------------*/
/**
 * @brief Converts ampers in [A] to ADC counts
 *
 * Function is used for setting up trigger threshold value, which is written into
 * appropriate FPGA register. This value needs to be specified in ADC counts, while
 * user specifies this information in Ampers. The resulting value is based on the
 * specified threshold voltage, maximal ADC voltage, calibrated and user specified
 * DC offsets.
 *
 * @param[in] current        Current, specified in [A]
 * @param[in] adc_max_v      Maximal ADC voltage, specified in [V]
 * @param[in] calib_dc_off   Calibrated DC offset, specified in ADC counts
 * @param[in] user_dc_off    User specified DC offset, specified in [V]
 * @param[in] k              Current probe factor for calculating input voltage
 * @retval    int            ADC counts
 */
int pwr_fpga_cnv_a_to_cnt(float current, float adc_max_v,
                          int calib_dc_off, float user_dc_off,
                          float k)
{
    int adc_cnts = 0;
    ...
    /* check and limit the specified voltage argument	s towards */
    /* maximal voltages which can be applied on ADC inputs     */
    if(voltage > adc_max_v)
        voltage = adc_max_v;
    else if(voltage < -adc_max_v)
 ...
 */
float pwr_fpga_cnv_cnt_to_v(int cnts, float adc_max_v,
                            int calib_dc_off, float user_dc_off,
                            float diff_probe_att)
{
    int m;
    ...
    /* check sign */
    if(cnts & (1<<(c_pwr_fpga_adc_bits-1))) {
        /* negative number */
        m = -1 *((cnts ^ ((1<<c_pwr_fpga_adc_bits)-1)) + 1);
    } else {
        /* positive number */
        m = cnts;
    }
    ...
    if(m < (-1 * (1<<(c_pwr_fpga_adc_bits-1))))
        m = (-1 * (1<<(c_pwr_fpga_adc_bits-1)));
    else if(m > (1<<(c_pwr_fpga_adc_bits-1)))
        m =  (1<<(c_pwr_fpga_adc_bits-1));
...
/*----------------------------------------------------------------------------*/
/**
 * @brief Converts ADC counts to ampers [A]
 *
 * Function is used to publish captured signal data to external world in user units.
 * Calculation is based on maximal voltage, which can be applied on ADC inputs and
 * calibrated and user defined DC offsets.
 ...
 * @param[in] calib_dc_off   Calibrated DC offset, specified in ADC counts
 * @param[in] user_dc_off    User specified DC offset, specified in [V]
 * @param[in] k              Current transducer factor
 * @retval    float          Signal Value, expressed in user units [A]
 */
float pwr_fpga_cnv_cnt_to_a(int cnts, float adc_max_v,
                            int calib_dc_off, float user_dc_off,
                            float k)
{
    int m;
    ...
    volts = (m * adc_max_v / (float)(1<<(c_pwr_fpga_adc_bits-1)));
 ...
 * This calculation is based on calibrated Front End Full Scale Gain setting.
 *
...
float pwr_fpga_calc_adc_max_v(uint32_t fe_gain_fs)
{
    ...
    max_adc_v = 
        fe_gain_fs/(float)((uint64_t)1<<32) * 100;
